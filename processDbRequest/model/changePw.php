...
if (isset($_POST['pwOne']) && isset($_POST['pwTwo'])) {
    ...
    if($pw1 !== $pw2) {
        $_SESSION["errorPWmismatch"] = " <p class='text-center text-danger m-1' style='display: block;' id='overallMsg'>Password Mismatch!</p>";
        $noErrors = false;
    ...
    if(strlen($pw1) < 8 || strlen($pw2) < 8) {
        $_SESSION["errorLength"] = " <p class='text-center text-danger m-1' style='display: block;' id='overallMsg'>Password length must be as least 8 characters</p>";
        $noErrors = false;
        header("Location: ../../HTML/updatePw.php");
        exit;
    }
